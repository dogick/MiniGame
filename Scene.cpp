#include "stdafx.h"
#include "Const.h"
#include "Scene.h"

void InitializeScene(Scene & scene)
{
	InitializeWall(scene.walls);
	InitializeCenterLine(scene.centerLines);
}

void InitializeCenterLine(sf::RectangleShape & line)
{
	line.setSize(sf::Vector2f(WINDOW_HEIGHT, 2));
	line.rotate(90);
	line.setPosition(WINDOW_WIDTH / 2, 0);
}

Wall::Wall(sf::Vector2f const& position, sf::Vector2f const& size, sf::Vector3i const& color)
{
	rectangle.setPosition(position);
	rectangle.setSize(size);
	rectangle.setFillColor(sf::Color(color.x, color.x, color.z));
	rectangle.setOutlineThickness(2);
}

void InitializeWall(std::vector<Wall*> & walls)
{
	walls.push_back(new Wall({ 0, 0 }, { float(WINDOW_WIDTH), 50 }, { 24, 56,177 }));
	walls.push_back(new Wall({ 0, 0 }, { 10, float(WINDOW_HEIGHT) }, { 24, 56,177 }));
	walls.push_back(new Wall({ float(WINDOW_WIDTH - 10), 0 }, { 10, float(WINDOW_HEIGHT) }, { 24, 56,177 }));
	walls.push_back(new Wall({ 0, float(WINDOW_HEIGHT - 10) }, { float(WINDOW_WIDTH), 10 }, { 24, 56,177 }));
}
